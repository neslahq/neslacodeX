...
class ParallelDims:
    ...
    def __post_init__(self):
        self.p_map = OrderedDict() # arranged from outermost to innermost dimensions
        for strategy in self.order:
            if strategy == 'tp':
                self.p_map[strategy] = self.tp
            elif strategy == 'pp':
    ...
    def build_mesh(self) -> DeviceMesh:
        ...
        if self.ep > 1:
            ...
            if len(self.order) > 0:
                return self._build_dynamic_mesh_without_ep()
    ...
    def _build_dynamic_mesh_without_ep(self) -> DeviceMesh:
        dims = []
        names = []
        for name, d in self.p_map.items():
            if d > 1:
                dims.append(d)
                names.append(name)
        ...
        # I think this part of the code is relevant to the following paragraph from parallelism mesh zoo:
        # CP has a pretty interesting interaction with FSDP. Both DP and CP shard the input data (on batch and sequence respectively). 
        # It's pretty common when you do FSDP to just shard over both "dp" ("dp_shard" in HSDP) and "cp". In torchtitan, we create a 
        # flattened mesh dim "dp_shard_cp" specifically for FSDP sharding (a flattened mesh dim is what happens if you take your mess and "forget" 
        # about some of the structure; e.g., if you were to do an all-gather, you just all-gather over all the flattened axes). In the HSDP world, 
        # "dp_cp" is still a useful concept because this is the combination of axes you want to all-reduce over to, e.g., compute the global average loss.
        ...
        if dp_mesh_dim_names != []:
            # split batch of data across dp dim and split across dp_shard dim
            mesh[tuple(dp_mesh_dim_names)]._flatten(mesh_dim_name="dp")
        if dp_shard_cp_mesh_dim_names != []:
            # all gather the weights on the dp_shard dim and cp dim before fwd computation and unshard the weights after
            # likewise for reduce-scatter during backward computation
            mesh[tuple(dp_shard_cp_mesh_dim_names)]._flatten(
                mesh_dim_name="dp_shard_cp"
            )
        if dp_cp_mesh_dim_names != []:
            # all reduce the loss for dp dim, dp_shard_dim and cp dim
            mesh[tuple(dp_cp_mesh_dim_names)]._flatten(mesh_dim_name="dp_cp")
